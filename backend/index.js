...
app.post("/api/filter", (req, res) => {
  ...
  const filteredData = csvData.filter(
    (row) => row.Domain === domain && row.Level === level
  );
  ...
  if (filteredData.length > 0) {
    ...
    res
      .status(404)
      .json({
        message: "No milestones found for the selected domain and level.",
      });
  }
});
  ...
  if (action === "Lookup Milestone") {
    ...
    if (domainMilestones.length > 0) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "No milestones found for this domain and level",
        });
    }
  }
